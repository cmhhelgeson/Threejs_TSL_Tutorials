...
import { color, uniform, vec2, pass, linearDepth, normalWorld, triplanarTexture, texture, objectPosition, screenUV, viewportLinearDepth, viewportDepthTexture, viewportSharedTexture, blur, mx_worley_noise_float, positionWorld, timerLocal } from 'three/tsl';
import { GLTFLoader } from 'three/addons/loaders/GLTFLoader.js';
import { OrbitControls } from 'three/addons/controls/OrbitControls.js';
import { GUI } from 'three/addons/libs/lil-gui.module.min.js';
import Stats from 'three/addons/libs/stats.module.js';

let camera, waterScene, backdropAlphaScene, renderer;
...
function init() {
	...
	//waterScene.fog = new THREE.Fog( 0x0487e2, 7, 25 );
	waterScene.backgroundNode = normalWorld.y.mix( color( 0x0487e2 ), color( 0x0066ff ) );
	camera.lookAt( 0, 1, 0 );
	...
	waterScene.add( sunLight );
	waterScene.add( skyAmbientLight );
	...
	sceneFolder.add( effectController, 'fog' ).onChange( () => {
		...
		waterScene.fog = effectController.fog === true ? new THREE.Fog( 0x0487e2, 7, 25 ) : null;
	...
	lightsFolder.add( effectController, 'sceneBackground', [ 'standard', 'dark', 'none' ] ).onChange( () =>{
	    ...
	    switch ( effectController.sceneBackground ) {
			...
			case 'dark': {
	    		...
	    		waterScene.backgroundNode = normalWorld.y.mix( color( 0x100f30 ), color( 0x0066ff ) );
	    		break;
	    	...
	    	case 'none': {
	    		...
	    		waterScene.backgroundNode = color( 0 );
	    		break;
	    ...
	    waterScene.backgroundNode.needsUpdate = true;
